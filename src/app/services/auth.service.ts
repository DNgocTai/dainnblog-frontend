...
export class AuthService {
  ...
  login(data): Observable<any> {
    return this._http.post<any>(`${environment.apiUrl}/user/login`, data).pipe(
      tap((res) => {
  ...
  fetchUserData(): Observable<any> {
    return this._http.get<any>(`${environment.apiUrl}/user/me`).pipe(
      tap((res) => {
        this.$User.next({
          _id: res._id,
  ...
  getBloggerProfile(bloggerId: String) {
    return this._http
      .get(`${environment.apiUrl}/user/bloggerProfile/${bloggerId}`)
      .pipe(catchError((err) => this._errorService.handleError(err)));
  }
}
